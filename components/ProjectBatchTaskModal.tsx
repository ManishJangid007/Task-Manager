...
interface ProjectBatchTaskModalProps {
  projectId: string;
  onSubmit: (tasks: Array<{ title: string; date: string; priority: TaskPriority; parentTaskId?: string }>) => void;
  onCancel: () => void;
}
...
const ProjectBatchTaskModal: React.FC<ProjectBatchTaskModalProps> = ({ projectId, onSubmit, onCancel }) => {
  ...
  const handleAddRow = (preserveSubtaskStatus = false, insertAfterIndex?: number, explicitParentTaskId?: number) => {
    ...
    const lastDate = targetRow?.date || getTodayDateString();
  ...
  const handleMakeSubtask = (rowIndex: number) => {
    ...
    if (currentRow.parentTaskId) {
      // Find the parent row to get its date
      const parentRow = taskRows.find(r => r.id === currentRow.parentTaskId);
      setTaskRows(taskRows.map((row, idx) => 
        idx === rowIndex ? { ...row, parentTaskId: undefined, date: parentRow?.date || row.date } : row
      ));
    } else {
      ...
      setTaskRows(taskRows.map((row, idx) => 
        idx === rowIndex ? { ...row, parentTaskId: parentRow.id, date: parentRow.date } : row
      ));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // The parentTaskId will be the index in the parent tasks array
    const validRows = taskRows.filter(row => row.title.trim() !== '' && row.date);
    ...
    const tasksToSubmit: Array<{ title: string; date: string; priority: TaskPriority; parentTaskId?: string }> = [];
